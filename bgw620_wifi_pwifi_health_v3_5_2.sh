#!/bin/sh
# ============================================================
# BGW620 WiFi / PWiFi Health Analyzer V3.5.2
# Read-only diagnostics. No config changes are performed.
# V3.5.2: hardcoded /bin/scp upload to BCEO. No ssh, no mkdir, no XLSX conversion.
# ============================================================
...
exists(){ [ -x "/bin/$1" ] || [ -x "/usr/bin/$1" ] || [ -x "/sbin/$1" ] || [ -x "/usr/sbin/$1" ]; }
...
init_report(){
    ...
    log "CSV     : $CSV"
    log "LOG     : $LOG"
    csv_row system start script bgw620_wifi_pwifi_health_v3_5_2 INFO "Started BGW620 health analyzer"
}
...
cloud_channel(){
    ...
    if [ -x /bin/envoy-cmd ] || [ -x /usr/bin/envoy-cmd ]; then envoy-cmd get Device.WiFi.X_ATT_AFC. >> "$LOG" 2>&1; csv_row acs afc afc_status captured INFO "AFC status saved to log"; fi
}
...
dns_health(){
    ...
    if [ -x /bin/nslookup ] || [ -x /usr/bin/nslookup ]; then nslookup att.com >> "$LOG" 2>&1 && csv_row dns resolution att.com resolved PASS "nslookup succeeded" || csv_row dns resolution att.com failed WARN "nslookup failed"; else csv_row dns resolution nslookup not_found SKIP "nslookup unavailable"; fi
