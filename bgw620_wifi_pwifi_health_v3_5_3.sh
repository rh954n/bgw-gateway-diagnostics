#!/bin/sh
# ============================================================
# BGW620 WiFi / PWiFi Health Analyzer V3.5.3
# Read-only diagnostics. No config changes are performed.
# V3.5.3 updates:
#   - More visible on-screen summaries for Options 5, 6, 9, 10, and 99
#   - Hardcoded /bin/scp upload to BCEO. No ssh, no mkdir, no XLSX conversion.
#   - CSV and LOG are generated locally, then uploaded together with one SCP prompt.
# ============================================================
...
LAST_UPLOAD_STATUS="NOT_RUN"
LAST_BACKHAUL_RESULT="UNKNOWN"
LAST_ACS_RESULT="UNKNOWN"
LAST_MEMORY_RESULT="UNKNOWN"
LAST_DNS_RESULT="UNKNOWN"
...
exists_bin(){ [ -x "/bin/$1" ] || [ -x "/usr/bin/$1" ] || [ -x "/sbin/$1" ] || [ -x "/usr/sbin/$1" ]; }
...
init_report(){
    ...
    log "CSV     : $CSV"
    log "LOG     : $LOG"
    csv_row system start script bgw620_wifi_pwifi_health_v3_5_3 INFO "Started BGW620 health analyzer"
}

print_paths(){ echo ""; echo "CSV=$CSV"; echo "LOG=$LOG"; echo "SERIAL=$SERIAL_SAFE"; }

memory_health(){
    section "SYSTEM / MEMORY HEALTH"
    MEM_TOTAL="unknown"; MEM_USED="unknown"; MEM_FREE="unknown"; MEM_AVAIL="unknown"; SLAB_MB="unknown"; SUN_MB="unknown"; CMA_MB="unknown"
    if [ -x /usr/bin/free ] || [ -x /bin/free ]; then
        free -m >> "$LOG" 2>&1
        LINE=`free -m 2>/dev/null | awk '/^Mem:/ {print}'`
        MEM_TOTAL=`echo "$LINE"|awk '{print $2}'`; MEM_USED=`echo "$LINE"|awk '{print $3}'`; MEM_FREE=`echo "$LINE"|awk '{print $4}'`; BC=`echo "$LINE"|awk '{print $6}'`; MEM_AVAIL=`echo "$LINE"|awk '{print $7}'`
        [ -z "$MEM_AVAIL" ] && MEM_AVAIL=0
        LAST_MEMORY_RESULT=PASS; NOTE="Available memory healthy"
        if [ "$MEM_AVAIL" -lt 250 ]; then LAST_MEMORY_RESULT=FAIL; NOTE="Available memory below 250 MB"; elif [ "$MEM_AVAIL" -lt 500 ]; then LAST_MEMORY_RESULT=WARN; NOTE="Available memory below 500 MB"; fi
        csv_row memory free_m mem_total_mb "$MEM_TOTAL" INFO "Total RAM"
        csv_row memory free_m mem_used_mb "$MEM_USED" INFO "Used RAM"
        csv_row memory free_m mem_free_mb "$MEM_FREE" INFO "Free RAM"
        csv_row memory free_m buff_cache_mb "$BC" INFO "Buffer/cache RAM"
        csv_row memory free_m mem_available_mb "$MEM_AVAIL" "$LAST_MEMORY_RESULT" "$NOTE"
    else
        LAST_MEMORY_RESULT=SKIP
    ...
    if [ -r /proc/meminfo ]; then
        ...
        for key in MemAvailable MemFree Slab SUnreclaim CmaFree; do
            ...
            [ "$key" = "Slab" ] && SLAB_MB="$MB"
            [ "$key" = "SUnreclaim" ] && SUN_MB="$MB"
            [ "$key" = "CmaFree" ] && CMA_MB="$MB"
            RES=INFO; NOTE2="/proc/meminfo value"
            if [ "$key" = "SUnreclaim" ]; then
                RES=PASS; NOTE2="Kernel unreclaimable slab"
                if [ "$MB" -gt 700 ]; then RES=FAIL; elif [ "$MB" -gt 400 ]; then RES=WARN; fi
            fi
            csv_row memory proc_meminfo "$key" "${MB} MB" "$RES" "$NOTE2"
        done
    fi

    section "TOP MEMORY PROCESS SAMPLE"
    if [ -x /usr/bin/pidstat ] || [ -x /bin/pidstat ]; then
        pidstat -ruh >> "$LOG" 2>&1
    ...
    echo ""
    echo "============================================================"
    echo " MEMORY HEALTH SUMMARY"
    echo "============================================================"
    echo "Total Memory       : ${MEM_TOTAL} MB"
    echo "Used Memory        : ${MEM_USED} MB"
    echo "Free Memory        : ${MEM_FREE} MB"
    echo "Available Memory   : ${MEM_AVAIL} MB"
    echo "Slab Memory        : ${SLAB_MB} MB"
    echo "SUnreclaim         : ${SUN_MB} MB"
    echo "CMA Free           : ${CMA_MB} MB"
    echo "Result             : ${LAST_MEMORY_RESULT}"
...
safe_backhaul(){
    ...
    nvram show 2>/dev/null | grep -E "map_agent_active_bh_type|map_onboarded|map_onboarded_first_time|multiap_mode|wbd_ifnames|wl[0-2]_band_flag|map_bh|Backhaul" > "$TMP" 2>/dev/null
    cat "$TMP" >> "$LOG" 2>&1
    ...
    ON1=`grep '^map_onboarded_first_time=' "$TMP"|tail -1|cut -d= -f2`; [ -z "$ON1" ] && ON1=unknown
    MM=`grep '^multiap_mode=' "$TMP"|tail -1|cut -d= -f2`; [ -z "$MM" ] && MM=unknown
    WBD=`grep '^wbd_ifnames=' "$TMP"|tail -1|cut -d= -f2-`; [ -z "$WBD" ] && WBD=unknown
    WL0=`grep '^wl0_band_flag=' "$TMP"|tail -1|cut -d= -f2`; WL1=`grep '^wl1_band_flag=' "$TMP"|tail -1|cut -d= -f2`; WL2=`grep '^wl2_band_flag=' "$TMP"|tail -1|cut -d= -f2`
    LAST_BACKHAUL_RESULT=PASS; NOTE="Backhaul/EasyMesh data collected"
    [ "$ON" != 1 ] && LAST_BACKHAUL_RESULT=WARN && NOTE="map_onboarded not 1 or unknown"
    csv_row backhaul safe_nvram backhaul_type "$BH" "$LAST_BACKHAUL_RESULT" "$NOTE"
    csv_row backhaul safe_nvram map_onboarded "$ON" INFO "EasyMesh onboarded flag"
    csv_row backhaul safe_nvram map_onboarded_first_time "$ON1" INFO "EasyMesh initially onboarded flag"
    ...
    csv_row backhaul safe_nvram wl0_band_flag "$WL0" INFO "Radio band flag"
    csv_row backhaul safe_nvram wl1_band_flag "$WL1" INFO "Radio band flag"
    csv_row backhaul safe_nvram wl2_band_flag "$WL2" INFO "Radio band flag"
    rm -f "$TMP"

    echo ""
    echo "============================================================"
    echo " BACKHAUL / EASYMESH SUMMARY"
    echo "============================================================"
    echo "Backhaul Type      : $BH"
    echo "Map Onboarded      : $ON"
    echo "First Onboarded    : $ON1"
    echo "MultiAP Mode       : $MM"
    echo "WBD Interfaces     : $WBD"
    echo "Band Flags         : wl0=$WL0 wl1=$WL1 wl2=$WL2"
    echo "Result             : $LAST_BACKHAUL_RESULT"
    echo "Details saved to   : $LOG"
}
...
wifi_distribution(){
    ...
    awk '/^---/ {iface=$2; band=$3; count[iface]=0; b[iface]=band; order[++n]=iface; next} /^assoclist/ {count[iface]++} END {for(i=1;i<=n;i++) print order[i]","b[order[i]]","count[order[i]]}' "$TMP" | while IFS=, read I B C; do [ -n "$I" ] && { csv_row wifi client_distribution "$I $B" "$C" INFO "Associated clients"; echo "  $I $B clients: $C"; }; done
...
wifi_status(){
    ...
    for IF in wl0 wl1 wl2; do
        ...
        echo "$IF channel=$CH utilization=${U}%"
        rm -f "$TMP"
    done
...
bss_role_scan(){
    ...
    for IF in wl0 wl0.1 wl0.2 wl0.3 wl0.4 wl0.5 wl0.6 wl0.7 wl1 wl1.1 wl1.2 wl1.3 wl1.4 wl1.5 wl1.6 wl1.7 wl2 wl2.1 wl2.2 wl2.3 wl2.4 wl2.5 wl2.6 wl2.7; do
        ...
        echo "$IF: $OUT" >> "$LOG"
        echo "$OUT"|grep -q 'Backhaul-BSS' && { BH_COUNT=`expr $BH_COUNT + 1`; csv_row backhaul bss_role_scan "$IF" Backhaul-BSS PASS "Backhaul BSS detected"; echo "  $IF : Backhaul-BSS"; wl -i "$IF" bs_data -noreset >> "$LOG" 2>&1; continue; }
        echo "$OUT"|grep -q 'Fronthaul-BSS' && { FH_COUNT=`expr $FH_COUNT + 1`; csv_row backhaul bss_role_scan "$IF" Fronthaul-BSS INFO "Fronthaul BSS detected"; echo "  $IF : Fronthaul-BSS"; }
    done
    echo "Backhaul BSS Count : $BH_COUNT"
    echo "Fronthaul BSS Count: $FH_COUNT"
    echo "Details saved to   : $LOG"
}
...
wifi7_mlo(){
    ...
    echo "MLO Active         : $MLO"
    echo "MLD Links          : $LINKS"
    echo "Current Chanspec   : $CURC"
    echo "320MHz Evidence    : $W320"
    echo "EHT                : $EHTV"
    echo "Result             : $R"
    echo "Details saved to   : $LOG"
    rm -f "$TMP" "$CUR" "$EHT"
...
acs_health(){
    ...
    for IF in wl0 wl1 wl2; do
        ...
        IO=`grep -c IOCTL "$REC" 2>/dev/null`; NO=`grep -c NONACS "$REC" 2>/dev/null`; EX=`grep -c 'Invalid:EXCLUDE' "$CS" 2>/dev/null`; [ -z "$EX" ] && EX=0
        TOTAL_EX=`expr $TOTAL_EX + $EX`
        ...
        echo "$IF last_trigger=$TR last_chanspec=$LC ioctl=$IO nonacs=$NO invalid_exclude=$EX"
        rm -f "$REC" "$CS" "$CH"
    done
    ...
    echo "Total Invalid Exclude Count: $TOTAL_EX"
    echo "Result                     : $LAST_ACS_RESULT"
    echo "Details saved to           : $LOG"
}

cloud_channel(){
    section "CLOUD CHANNEL MANAGEMENT"
    if [ -x /bin/eco_envoy ] || [ -x /usr/bin/eco_envoy ]; then
        ...
        echo "Cloud channel data: captured"
    else
        csv_row acs cloud_channel eco_envoy_status not_found SKIP "eco_envoy not available"
        echo "eco_envoy not available, skipped"
    fi
    if [ -x /bin/envoy-cmd ] || [ -x /usr/bin/envoy-cmd ]; then envoy-cmd get Device.WiFi.X_ATT_AFC. >> "$LOG" 2>&1; csv_row acs afc afc_status captured INFO "AFC status saved to log"; echo "AFC status: captured"; fi
    echo "Details saved to: $LOG"
}
...
ceventc_health(){
    ...
    echo "Assoc Events       : $ASSOC"
    echo "Authorized Events  : $AUTHZ"
    echo "Disassoc Events    : $DIS"
    echo "Deauth Events      : $DE"
    echo "SAE Events         : $SAE"
    echo "Result             : $R"
    echo "Details saved to   : $LOG"
    rm -f "$TMP"
...
dns_health(){
    ...
    DNS=unknown; DNSRES=SKIP; IC=unknown; NAT=unknown
    ...
    if [ -x /bin/nslookup ] || [ -x /usr/bin/nslookup ]; then
        nslookup att.com >> "$LOG" 2>&1 && { DNSRES=PASS; csv_row dns resolution att.com resolved PASS "nslookup succeeded"; } || { DNSRES=WARN; csv_row dns resolution att.com failed WARN "nslookup failed"; }
    else
        csv_row dns resolution nslookup not_found SKIP "nslookup unavailable"
    fi
    if [ -x /bin/tr69 ] || [ -x /usr/bin/tr69 ]; then
        ...
        csv_row dns icmp_suppression tr69 not_found SKIP "tr69 unavailable"
        NR=SKIP
    fi
    LAST_DNS_RESULT="$DNSRES"
    echo ""
    echo "============================================================"
    echo " DNS / NAT HEALTH SUMMARY"
    echo "============================================================"
    echo "Primary DNS        : $DNS"
    echo "DNS Resolution     : $DNSRES"
    echo "ICMP Suppression   : $IC"
    echo "NAT MaxSessions    : $NAT"
    echo "NAT Result         : $NR"
    echo "Details saved to   : $LOG"
}
...
stability_health(){
    ...
    for CMD in "show process-restart" "show hbmon" "show crash" "show crashlog" "show kfm crash"; do
        if [ -x /bin/cshell ] || [ -x /usr/bin/cshell ]; then cshell -c "$CMD" >> "$LOG" 2>&1; csv_row stability cshell "$CMD" captured INFO "Saved to log"; else csv_row stability cshell "$CMD" skipped SKIP "cshell not found"; fi
    done
    ...
    echo "Stability details saved to: $LOG"
}

client_lookup(){
    echo -n "Enter client MAC to lookup: "; read MAC; [ -z "$MAC" ] && return
    section "CLIENT LOOKUP $MAC"
    if [ -x /bin/cshell ] || [ -x /usr/bin/cshell ]; then cshell -c "show ip lan" 2>/dev/null | grep -i "$MAC" >> "$LOG"; fi
    ceventc -m "$MAC" dump >> "$LOG" 2>&1
    csv_row client lookup mac "$MAC" INFO "Client lookup saved to log"
    echo "Client lookup saved to: $LOG"
}

overall(){
    section "OVERALL HEALTH SUMMARY"
    W=`grep -c ',"WARN",' "$CSV" 2>/dev/null`; F=`grep -c ',"FAIL",' "$CSV" 2>/dev/null`; [ -z "$W" ] && W=0; [ -z "$F" ] && F=0
    OVERALL_RESULT=PASS; [ "$F" -gt 0 ] && OVERALL_RESULT=FAIL; [ "$F" -eq 0 ] && [ "$W" -gt 0 ] && OVERALL_RESULT=PASS_WITH_WARNINGS
    log "WARN rows: $W"; log "FAIL rows: $F"; log "Overall : $OVERALL_RESULT"
    csv_row summary overall warn_count "$W" INFO "Total WARN rows"
    csv_row summary overall fail_count "$F" INFO "Total FAIL rows"
    csv_row summary overall overall_health "$OVERALL_RESULT" "$OVERALL_RESULT" "Overall result"
}

upload_bceo(){
    section "UPLOAD TO BCEO USING HARDCODED /bin/scp"
    CSVN=`basename "$CSV"`; LOGN=`basename "$LOG"`
    log "Target: $BCEO_USER@$BCEO_HOST:$BCEO_DIR/"
    log "Using SCP binary: $SCPBIN"
    log "No ssh/mkdir. No XLSX conversion. Enter BCEO password if prompted."
    LAST_UPLOAD_STATUS=NOT_RUN
    if [ ! -x "$SCPBIN" ]; then
        csv_row upload bceo_scp scp_binary missing FAIL "/bin/scp not found or not executable"
        LAST_UPLOAD_STATUS=FAIL
        echo "ERROR: /bin/scp not found or not executable"
        return 1
    fi
    echo ""; echo "============================================================"; echo " UPLOAD TO BCEO"; echo "============================================================"
    echo "Target: $BCEO_USER@$BCEO_HOST:$BCEO_DIR/"
    echo "Uploading CSV and LOG together."
    echo "Enter BCEO password when prompted."
    echo "CSV: $CSVN"
    echo "LOG: $LOGN"
    "$SCPBIN" -P "$BCEO_PORT" "$CSV" "$LOG" "$BCEO_USER@$BCEO_HOST:$BCEO_DIR/"
    RC=$?
    if [ "$RC" -eq 0 ]; then
        LAST_UPLOAD_STATUS=PASS
        echo "Upload Complete"; echo "Files uploaded to: $BCEO_DIR/"
        csv_row upload bceo_scp csv_upload "$CSVN" PASS "$BCEO_DIR/$CSVN"
        csv_row upload bceo_scp log_upload "$LOGN" PASS "$BCEO_DIR/$LOGN"
        csv_row upload bceo_scp overall_upload uploaded PASS "$BCEO_DIR/"
    else
        LAST_UPLOAD_STATUS=FAIL
        echo "Upload Failed"; echo "SCP return code: $RC"
        csv_row upload bceo_scp overall_upload failed FAIL "scp return code $RC"
    fi
...
option99(){
    ...
    log "CSV: $CSV"; log "LOG: $LOG"
    echo ""
    echo "============================================================"
    echo " BGW620 HEALTH SUITE COMPLETE"
    echo "============================================================"
    echo "Serial             : $SERIAL_SAFE"
    echo "Overall Health     : $OVERALL_RESULT"
    echo "Upload Status      : $LAST_UPLOAD_STATUS"
    echo "CSV File           : $CSV"
    echo "LOG File           : $LOG"
    echo "BCEO Directory     : $BCEO_DIR/"
    echo "XLSX               : Skipped; open CSV in Excel and Save As .xlsx"
    echo "============================================================"
}
